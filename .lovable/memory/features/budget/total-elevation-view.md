...
16. **Hastiales con visibilidad mixta**: Las secciones gable NO se fusionan en una sola; se mantienen separadas con su propio `effectiveWallType`. El renderizado usa un triángulo compartido (clipPath) para todas las secciones del hastial. Las secciones invisibles se recortan con un rect blanco dentro del triángulo. Las visibles muestran bloques clippeados. Cada sección incluye `gablePeakHeight`, `gableTotalLength` y `gableSectionStart` para geometría correcta.
