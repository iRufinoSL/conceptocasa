...
export function FloorPlanCanvas2D({
  ...
  const renderSvgContent = () => (
    <>
      ...
      {/* Ridge line (viga cumbrera) — red line at midpoint of width for dos_aguas roofs */}
      {plan.roofType === 'dos_aguas' && (() => {
        const ridgeX = (plan.width / 2) * SCALE;
        const extT = plan.externalWallThickness;
        const overhang = plan.roofOverhang || 0;
        const y1 = -(extT + overhang) * SCALE - 20;
        const y2 = (plan.length + extT + overhang) * SCALE + 20;
        return (
          <g pointerEvents="none">
            <line x1={ridgeX} y1={y1} x2={ridgeX} y2={y2}
              stroke="#dc2626" strokeWidth={2.5} strokeDasharray="10 4" opacity={0.9} />
            <text x={ridgeX + 8} y={y1 + 14}
              fontSize={9} fill="#dc2626" opacity={0.95} fontWeight="700">
              ▲ Cumbrera
            </text>
          </g>
        );
