...
export function GridPdfExport({ title, subtitle, containerRef, size = 'sm' }: GridPdfExportProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      doc.text(`Escala: ${scalePct}%  ·  ${dateStr}`, A4_W - MARGIN, MARGIN + 5, { align: 'right' });
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">Exportar a PDF</DialogTitle>
          </DialogHeader>
