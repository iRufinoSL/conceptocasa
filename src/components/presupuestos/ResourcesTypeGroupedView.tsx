...
export function ResourcesTypeGroupedView({
  ...
  const orderedTypes = useMemo(() => {
    ...
    const typesWithResources = Object.keys(groupedByType).filter(type => (groupedByType[type]?.length || 0) > 0);
                ...
                {isExpanded && typeResources.map(resource => {
                  ...
                  const activityOptions = [...activities]
                    .sort((a, b) => {
                      ...
                      const idA = `${phaseA?.code || ''} ${a.code}.-${a.name}`;
                      const idB = `${phaseB?.code || ''} ${b.code}.-${b.name}`;
